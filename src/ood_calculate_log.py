...
def get_curve(dir_name, stypes=['Baseline', 'Gaussian_LDA']):
    ...
    tnr_at_tpr95 = dict()
    for stype in stypes:
        known = np.genfromtxt('{}/confidence_{}_In.txt'.format(dir_name, stype), delimiter=',', filling_values=np.nan)
        novel = np.genfromtxt('{}/confidence_{}_Out.txt'.format(dir_name, stype), delimiter=',', filling_values=np.nan)
        known.sort()
        novel.sort()
